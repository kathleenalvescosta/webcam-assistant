## An alternate implementation of the OCR using Transformers for the TTS system
import cv2
import pytesseract
import threading
import torch
...
east_net = cv2.dnn.readNet(
    "/Users/kathleenalvescosta/Desktop/school/independentstudy/frozen_east_text_detection.pb")
...
def speak_text(text: str):
    ...
    with lock:
        ...
        wav = speech.cpu().numpy().astype(np.float32)

        peak = max(1e-5, float(np.max(np.abs(wav))))
        wav = wav / peak

        fade_len = int(0.02 * 16000)  
...
def detect_text_regions_east(image, net, conf_threshold=0.5):
    ...
    for y in range(num_rows):
        ...
        for x in range(num_cols):
            ...
            boxes.append(
                (int(start_x * r_w), int(start_y * r_h), int(end_x * r_w), int(end_y * r_h))
            )
...
def ocr_thread(gray_frame, rois):
    ...
    for x1, y1, x2, y2 in rois:
        ...
        data = pytesseract.image_to_data(
            roi, output_type=pytesseract.Output.DICT, config='--oem 3 --psm 7'
        )
    ...
    if text_lines:
        combined_text = " ".join([t for _, _, t in text_lines])
